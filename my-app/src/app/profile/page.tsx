...
export default function ProfilePage() {
  ...
  /**
   * DETAILS VIEW: Triggered when clicking "View All" on Following or Likes
   */
  const renderDetailsPage = () => (
    <div className="container mx-auto px-8 py-10 animate-in fade-in duration-500">
      ...
      <div className="py-20 text-center flex flex-col items-center">
        {detailTab === "Likes" && (
            <p className="text-2xl font-bold text-zinc-500 uppercase">YOU HAVE NO LIKES YET.</p>
          ) }
        
        {detailTab === "Followers" && (
          <p className="text-2xl font-bold text-zinc-500 uppercase">NO ONE IS FOLLOWING YOU YET.</p>
        )}

        {detailTab === "Following" && (
          following.length === 0 ? (
            <p className="text-2xl font-bold text-zinc-500 uppercase">YOU AREN'T FOLLOWING ANYONE YET.</p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-5 gap-8 w-full max-w-5xl">
              {following.map((user: any) => (
                <div key={user.id} className="flex flex-col items-center gap-3">
                  <div className="w-24 h-24 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center font-bold text-xl uppercase">
                    {user.name?.charAt(0)}
                  </div>
        ...
        <button
          ...
          className="mt-12 bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-zinc-200 transition-all uppercase text-sm"
        >
          ← Back to Profile
        </button>
      </div>
          ...
          <>
              ...
              <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-center text-center md:text-left mt-2">
                ...
                <div className="flex flex-col gap-1.5 items-center md:items-start">
                  <div className="flex flex-col md:flex-row items-center gap-2 bg-black px-3 py-1 w-fit">
                    ...
                    {accountType === "ARTIST" && (
                      <span className="bg-zinc-800 text-zinc-400 text-[10px] md:text-[12px] px-2 py-1 rounded-sm font-black uppercase border border-zinc-700/50 shadow-sm shrink-0">Artist</span>
                    )}
                  </div>
                  {location && (
                  <div className="flex items-center gap-1 mt-0.5">
                    <p className="text-neutral-400 text-[10px] md:text-xs bg-black px-2 py-1 w-fit font-bold uppercase">{location}</p>
                  </div>
                  )}
                </div>
              </div>
            ...
            <div className="border-b border-zinc-800 bg-[#121212] sticky top-0 z-40 overflow-x-auto">
              <div className="container mx-auto px-4 md:px-8 flex justify-between items-center h-12">
                <ul className="flex gap-8 text-[14px] font-bold text-zinc-400 h-full">
                  {tabs?.map((tab: string) => (
                    <li key={tab} onClick={() => setActiveTab(tab)} className={`cursor-pointer transition-colors h-full flex items-center border-b-2 ${activeTab === tab ? "text-white border-white" : "border-transparent hover:text-white"}`}>
                      {tab}
                    </li>
                  ))}
                </ul>
            ...
            <div className="container mx-auto px-8 py-12 flex flex-col lg:flex-row gap-12 text-left">
              ...
              <div className="flex-1 text-center py-20 border-r border-zinc-900/50 pr-12 flex flex-col items-center justify-center">
                <p className="text-zinc-500 text-xl font-bold mb-6 uppercase">
              ...
              <div className="w-full lg:w-[320px] space-y-10">
                ...
                {/* Likes Sidebar Preview */}
                

                {/* Following Sidebar Preview */}
                {following.length > 0 && (
                  <div className="space-y-4 border-t border-zinc-900 pt-6">
                    <div className="flex justify-between items-center text-zinc-500 text-[13px] border-b border-zinc-900 pb-2">
                      <p className="flex items-center gap-2 font-bold uppercase">👥 {following.length} Following</p>
                      <button onClick={() => { setViewState("details"); setDetailTab("Following"); }} className="hover:text-white transition-colors font-bold uppercase text-[11px]">View all</button>
                    </div>
