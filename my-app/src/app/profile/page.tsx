...
const {
    ...
    activeTab, setActiveTab, tabs, viewState, setViewState,
    detailTab, setDetailTab, isShareOpen, setIsShareOpen,
    accountType, isEditOpen, setIsEditOpen, followersCount,
    followingCount, tracksCount,
    shareTab, setShareTab, isShortened, setIsShortened,
    copied, copyToClipboard, shortLink, longLink, showSuccessToast,
    links, favoriteGenres // أضفنا links و favoriteGenres هنا
  } = controller;
  ...
  const renderDetailsPage = () => (
    <div className="container mx-auto px-8 py-10 animate-in fade-in duration-500">
      ...
      <div className="py-20 text-center flex flex-col items-center">
        {detailTab === "Likes" && <p className="text-2xl font-bold text-zinc-500">YOU HAVE NO LIKES YET.</p>}
        {detailTab === "Followers" && <p className="text-2xl font-bold text-zinc-500">NO ONE IS FOLLOWING YOU YET.</p>}
        {detailTab === "Following" && (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="font-bold text-xl">Travis Scott <span className="text-blue-500">✓</span></p>
          </div>
        )}
        <button onClick={() => setViewState("profile")} className="mt-12 bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-zinc-200 transition-all uppercase">
          ← Back to Profile
        </button>
      </div>
    </div>
        ...
        <>
          ...
          <div className="relative w-full min-h-[260px] bg-[#d38b7d] p-4 md:p-6 flex flex-col md:flex-row items-center md:items-start justify-between gap-4">
            <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-center text-center md:text-left mt-2">
              ...
              <div className="w-[110px] h-[110px] md:w-[180px] md:h-[180px] rounded-full bg-zinc-400/30 flex items-center justify-center border border-white/10 shadow-2xl overflow-hidden relative group cursor-pointer">
                <span className="text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity absolute inset-0 flex items-center justify-center bg-black/40">Update image</span>
              ...
              <div className="flex flex-col gap-1.5 items-center md:items-start">
                <div className="flex flex-col md:flex-row items-center gap-2 bg-black px-3 py-1 w-fit">
                  ...
                  {accountType === "ARTIST" && (
                    <span className="bg-zinc-800 text-zinc-400 text-[10px] md:text-[12px] px-2 py-1 rounded-sm font-black uppercase border border-zinc-700/50 shadow-sm flex-shrink-0">Artist</span>
                  )}
                </div>
               {location && (
  <div className="...">
    <p className="...">{location}</p>
            ...
            <button className="bg-black/80 px-3 py-1 rounded text-[9px] md:text-[11px] font-bold border border-white/20 hover:bg-black transition-colors self-end md:self-start uppercase">Upload header image</button>
          </div>
          ...
          <div className="border-b border-zinc-800 bg-[#121212] sticky top-0 z-40 overflow-x-auto">
            <div className="container mx-auto px-4 md:px-8 flex justify-between items-center h-12 min-w-[600px] md:min-w-full">
              ...
              <ul className="flex gap-8 text-[14px] font-bold text-zinc-400 h-full">
                {tabs?.map((tab: string) => (
                  <li key={tab} onClick={() => setActiveTab(tab)} className={`cursor-pointer outline-none focus:outline-none transition-colors h-full flex items-center border-b-2 ${activeTab === tab ? "text-white border-white" : "border-transparent hover:text-white"}`}>
                    {tab}
                  </li>
                ))}
              </ul>
...
{favoriteGenres && favoriteGenres.length > 0 && (
  <div className="space-y-1 mt-6 border-t border-zinc-900 pt-4">
    <p className="text-zinc-500 text-[10px] font-bold uppercase mb-1">Favorite Genres</p>
    {favoriteGenres.map((g) => (
      <p key={g} className="text-sm font-bold text-white flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-white"></span>{g}
      </p>
    ))}
  </div>
)}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-zinc-500 text-[13px] border-b border-zinc-900 pb-2">
                  <p className="flex items-center gap-2 font-bold uppercase">👥 1 Following</p>
                  <button onClick={() => { setViewState("details"); setDetailTab("Following"); }} className="hover:text-white transition-colors font-bold uppercase">View all</button>
                </div>
                <div className="flex items-center gap-3 p-2 hover:bg-zinc-900/40 rounded transition-all cursor-pointer">
                  ...
                  <p className="text-sm font-bold">Travis Scott <span className="text-blue-400">✓</span></p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#1a1a1a] w-full max-w-[500px] rounded-sm border border-[#333] shadow-2xl overflow-hidden relative">
            ...
            <div className="flex border-b border-[#333]">
              <button onClick={() => setShareTab("Share")} className={`px-6 py-4 text-sm font-bold transition-all uppercase ${shareTab === "Share" ? "text-white border-b-2 border-white" : "text-zinc-500 hover:text-white"}`}>Share</button>
              <button onClick={() => setShareTab("Message")} className={`px-6 py-4 text-sm font-bold transition-all uppercase ${shareTab === "Message" ? "text-white border-b-2 border-white" : "text-zinc-500 hover:text-white"}`}>Message</button>
            </div>
              ...
              {shareTab === "Share" ? (
                <>
                  ...
                  <div className="flex gap-4 mb-10">
                    <div title="Twitter" className="w-[50px] h-[50px] rounded-full bg-[#1DA1F2] flex items-center justify-center cursor-pointer hover:opacity-80 transition-all shadow-lg"><FaTwitter size={30}/></div>
                    <div title="Facebook" className="w-[50px] h-[50px] rounded-full bg-[#1877F2] flex items-center justify-center cursor-pointer hover:opacity-80 transition-all shadow-lg "><FaFacebook size ={30} /></div>
                    <div title="Tumblr" className="w-[50px] h-[50px] rounded-full bg-[#35465C] flex items-center justify-center cursor-pointer hover:opacity-80 transition-all shadow-lg"><TiSocialTumbler size={50} /></div>
                    <div title="Pinterest" className="w-[50px] h-[50px] rounded-full bg-[#E60023] flex items-center justify-center cursor-pointer hover:opacity-80 transition-all shadow-lg"><FaPinterest size ={30} /></div>
                    <div title="Email" className="w-[50px] h-[50px] rounded-full bg-[#333] flex items-center justify-center cursor-pointer hover:opacity-80 transition-all shadow-lg border border-zinc-700"><HiOutlineEnvelope size={30} /></div>
                  </div>
                  {/* Link Copy UI */}
                  <div className="space-y-4">
                    <div className="bg-[#111] border border-[#333] p-1 rounded flex items-center justify-between group">
                      <input readOnly value={isShortened ? shortLink : longLink} className="bg-transparent text-[13px] text-zinc-300 w-full outline-none px-2 font-bold" />
                      <button onClick={copyToClipboard} className={`px-4 py-1.5 rounded text-xs font-bold transition-all uppercase ${copied ? "bg-green-600" : "bg-white text-black"}`}>{copied ? "Copied!" : "Copy"}</button>
                    </div>
                    {/* Link Shortening Toggle */}
                    <div className="flex items-center gap-3">
                      <input type="checkbox" id="shorten" checked={isShortened} onChange={() => setIsShortened(!isShortened)} className="w-5 h-5 accent-white" />
                      <label htmlFor="shorten" className="text-sm text-white font-bold uppercase">Shorten link</label>
                    </div>
                  </div>
                </>
              ) : (
                ...
                <div className="space-y-4 animate-in fade-in duration-300 text-left">
                  ...
                  <div className="flex justify-end pt-2"><button className="bg-white text-black px-6 py-1.5 rounded font-bold text-sm hover:bg-zinc-200 uppercase">Send</button></div>
                </div>
              )}
            </div>
            {/* Modal Exit Button */}
            <button onClick={() => setIsShareOpen(false)} className="absolute top-4 right-4 text-zinc-500 hover:text-white text-xl uppercase">×</button>
          </div>
        </div>
      )}
      ...
      {showSuccessToast && (
        <div className="fixed top-20 right-10 z-[100] animate-in slide-in-from-right duration-300">
          <div className="bg-[#333] border border-zinc-700 p-4 flex items-center gap-4 shadow-2xl rounded-sm min-w-[300px]">
            <div className="w-12 h-12 bg-zinc-600 flex items-center justify-center rounded-sm"><span className="text-zinc-400 text-2xl">👤</span></div>
            <div>
