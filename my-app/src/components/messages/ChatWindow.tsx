  ...
  return (
    <section className="flex h-[calc(100vh-64px)] flex-1 flex-col p-8">
              ...
              <div key={message.id} className="flex gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex justify-between">
                    <p className="font-bold text-white">{isMe ? "Me" : selected.participant.display_name}</p>
                    <span className="text-xs text-zinc-500">{timeLabel(message.createdAt)}</span>
                  </div>

                  <p className="whitespace-pre-wrap text-sm text-zinc-300">{message.text}</p>

                  {message.sharedTrack && <SharedTrackCard track={message.sharedTrack} />}
                  {message.sharedPlaylist && <SharedPlaylistCard playlist={message.sharedPlaylist} />}
                </div>
              </div>
            );
          })}
      ...
      <MessageComposer
        ...
        onSend={(text, attachment) => sendMessage(selected.participant.id, text, attachment)}
      />
    </section>
  );
}