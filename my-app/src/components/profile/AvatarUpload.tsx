  ...
  return (
    <>
      ...
      <div className="relative w-40 h-40 md:w-48 md:h-48 rounded-full bg-zinc-400/30 flex items-center justify-center border border-white/10 shadow-2xl overflow-hidden group">
        ...
        {preview ? (
          <img src={preview} className="w-full h-full rounded-full object-cover" alt="Avatar" />
        ) : (
      ...
      {showEditor && (
        <div className="fixed inset-0 bg-white/30 backdrop-blur-sm flex items-center justify-center z-50">
          ...
          <div className="bg-zinc-900 p-5 rounded-xl flex flex-col items-center gap-4 w-[500px] shadow-2xl">
            {/* User info */}
            <h2 className="text-white font-bold text-2xl">{username}</h2>
            <p className="text-zinc-400 text-sm">{location}</p>
            <p className="text-zinc-300 text-xs">
              For best results, upload images of at least <strong>1000×1000 pixels</strong>. 2MB file-size limit.
            </p>

            {/* Cropper container */}
            <div className="relative w-[480px] h-[480px] rounded-full overflow-hidden bg-transparent shadow-lg">
              <Cropper
                image={tempImage!}
                crop={crop}
            ...
            <div className="flex items-center justify-between w-full mt-6">
              ...
              {isValidImage ? (
                <div className="flex items-center gap-2 flex-1">
                  <button onClick={() => setZoom((z) => Math.max(1, z - 0.1))}
                    className="bg-zinc-800 text-white px-3 py-2 rounded hover:bg-zinc-700 transition">-</button>
              ...
              <div className="flex gap-3 ml-6">
                <button onClick={() => setShowEditor(false)}
                  className="bg-zinc-800 text-white px-2 py-2 rounded text-sm hover:bg-zinc-700 transition">Cancel</button>
                <button onClick={handleSave}
                  className="bg-white text-black px-2 py-2 rounded font-bold text-sm hover:bg-gray-200 transition">Save</button>
              </div>
